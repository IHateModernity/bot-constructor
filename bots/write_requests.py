...
def get_requests_from_one_bot(path_db: str, name_bot: str = None, token_bot: str = None) -> list[Any]:
...
def write_file_answer(outfile: str, path_db: str, name_bot='', token_bot=''):
    ...
    with open(outfile, 'w', encoding='utf8') as f:
        ...
        if name_bot is None and token_bot is None:
            ...
            requests_for_bot = get_requests_from_one_bot(path_db, name_bot=name_bot)
        else:
            # получаем запросы бота по токену
            requests_for_bot = get_requests_from_one_bot(path_db, token_bot=token_bot)
